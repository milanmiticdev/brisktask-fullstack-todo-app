...
// Types
import type { TaskProps } from './../../types/prop.types';

const Task = ({ task, onDispatch }: TaskProps): JSX.Element => {
	const [taskName, setTaskName] = useState<string>(task.name ? task.name : '');
	...
	const handleDeleteTaskById = async () => await deleteTaskById({ taskId: task.id, userRole, token, dispatch: onDispatch, navigate });
